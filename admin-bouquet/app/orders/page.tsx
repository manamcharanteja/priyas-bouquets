...
export default function OrdersPage() {
  ...
  const load = (status: string) => {
    ...
    orderAPI.getAll(params).then((r) => { setOrders(r.data.orders || []); setLoading(false); }).catch(() => setLoading(false));
  };
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {STATUSES.map(s => (
          <button key={s} onClick={() => setFilter(s)}
            className={`px-4 py-2 rounded-full text-xs font-medium capitalize transition-colors ${filter === s ? 'bg-maroon-700 text-white' : 'bg-white border border-gray-200 text-gray-500 hover:border-maroon-700'}`}>
            {s}
          </button>
        ))}
      </div>
            ...
            <tbody>
              {loading ? [...Array(5)].map((_, i) => (
                ...
                <tr key={o._id} className="border-b border-gray-50 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-5 py-4"><p className="font-medium text-gray-800">{o.customer?.name}</p><p className="text-xs text-gray-400">{o.customer?.phone}</p></td>
                  <td className="px-5 py-4 text-gray-500">{o.items?.length}</td>
                  <td className="px-5 py-4 font-bold text-gray-800">₹{o.totalAmount?.toLocaleString()}</td>
                  ...
                  <td className="px-5 py-4"><Link href={`/orders/${o._id}`} className="text-maroon-700 text-xs hover:underline">View</Link></td>
                </tr>
              ))}
            </tbody>
          </table>
