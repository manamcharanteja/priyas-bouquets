...
export const orderAPI = {
  ...
  track: (email: string, orderId: string) =>
    api.get('/api/orders/track', { params: { email, orderId } }),
  updateStatus: (id: string, orderStatus: string) =>
    api.put(`/api/orders/${id}/status`, { orderStatus }),
  getStats: () => api.get('/api/orders/admin/stats'),
};

export const paymentAPI = {
  createOrder: (amount: number) => api.post('/api/payment/create-order', { amount }),
